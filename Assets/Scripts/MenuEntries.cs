...
public class MenuEntries : MonoBehaviour
{
    ...
    static void GenerateTerrain()
    {
        ...
        terrainGen.mercator = new LocalizedMercatorProjection();
        terrainGen.mercator.CalcAndSetOffset(min_lat, min_lon);
        terrainGen.srtm = new SRTM_Reader("Assets/SRTM");
        terrainGen.GenerateTerrain();
    }


    [MenuItem("OSM/blur terrain")]
    static void BlurFunc()
    {
        ...
        if(terrain)
        {
            ...
            var blurred = TerrainBlur.blur(heights, 7);
            terrain.terrainData.SetHeights(0,0, blurred);
